	...
	private void initRooms() {
		...
		for (int i = 0; i < rooms.length; i++) {
			for (int j = 0; j < rooms[i].length; j++) {
				rooms[i][j] = new Room(user = new User(), convertRoomNum(i, j),true, null);
...
//				System.out.print("입실시간 : " + rooms[i][j].getCheckInTime() +"\n"); // 생성확인 테스트
	...
	private boolean choiceMenu(int menu) {
		...
		case 2:
			...
			roomNum = sc.next();
			System.out.println("입실하실 분의 성함 입력 > ");
			name = sc.next();
			System.out.println("입실하실 분의 연령 입력 > ");
			age = sc.nextInt();
			System.out.println("입실하실 분의 성별 입력 > ");
			gen = sc.next();
			roomCheckIn(roomNum, name, age, gen);			
			break;
		case 3:
...
//		System.out.println(userObj.getGen());
	...
	private void roomCheckIn(String roomNum, String name, int age, String gen) {
		int floor = 0, room = 0;
		if (roomNum.length() > 3) { // 1004호 => 1004 => 10, 04 => 9, 3
			floor = Integer.parseInt(roomNum.substring(0, 2)) - 1;
			room = Integer.parseInt(roomNum.substring(2)) - 1;
		...
		if (rooms[floor][room].isEmpty()) {
			...
			rooms[floor][room].getUser().setName(name);
			rooms[floor][room].getUser().setAge(age);
			rooms[floor][room].getUser().setGen(gen);
